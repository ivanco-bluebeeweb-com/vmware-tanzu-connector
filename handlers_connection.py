...
async def connect_vmware_tanzu_connector(params: ConnectParams, ctx) -> ActionResult:
    ...
    return ActionResult.ok(rec, summary=f"Connected VMware Tanzu ({rec['label']}).")
...
async def disconnect_vmware_tanzu_connector(params: ConnectionIdParams, ctx) -> ActionResult:
    ...
    if params.connection_id:
        ...
        conns.clear()
    await _save_conns(ctx, conns)
    return ActionResult.ok({"success": True, "message": "Disconnected successfully."}, summary="Disconnected VMware Tanzu connection.")
