"""Connection lifecycle for VMware Tanzu Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from vmware_tanzu_client import VmwareTanzuClient
...
_SECRET = "vmware_tanzu_connections"
...
@chat.function(
    "connect_vmware_tanzu",
    "Connect VMware Tanzu account via credentials.",
    action_type="write",
    chain_callable=True,
    event="vmware-tanzu-connector.connect_vmware_tanzu",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_vmware_tanzu(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect VMware Tanzu Connector."""
    client = VmwareTanzuClient(api_key=params.api_key, base_url=params.base_url)
    await client.verify_auth()
    conns = await _load_connections(ctx)
    cid = f"conn_{uuid.uuid4().hex[:8]}"
    record = {
        "id": cid,
        "label": params.label or "VMware Tanzu Account",
...
@chat.function(
    "list_connections",
    "List connected VMware Tanzu accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_vmware_tanzu",
    "Disconnect VMware Tanzu account.",
    action_type="write",
    chain_callable=True,
    event="vmware-tanzu-connector.disconnect_vmware_tanzu",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_vmware_tanzu(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
